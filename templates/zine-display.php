...
function azm_migrate_page_php( $page ) {
	...
	if ( ! empty( $page['imageUrl'] ) ) {
		$elements[] = [
			'id' => uniqid(), 'type' => 'image',
			'x' => 8, 'y' => 8, 'w' => 84, 'h' => 60,
			'rotation' => 0, 'url' => $page['imageUrl'],
		];
	}
	return [
		'id'       => $page['id'] ?? uniqid(),
...
$pages = array_map( 'azm_migrate_page_php', $pages );
?>
	...
	<div class="azm-zine-reader">
		<div class="azm-zine-pages" id="azm-zine-pages">
			<?php foreach ( $pages as $i => $page ) :
				$bg       = esc_attr( $page['bgColor'] ?? '#ffffff' );
				$elements = $page['elements'] ?? [];
			?>
			<div class="azm-zine-page" data-index="<?php echo absint( $i ); ?>">
				<div class="azm-zine-canvas" style="background:<?php echo esc_attr( $page['bgColor'] ?? '#ffffff' ); ?>;">
				...
				?>
					...
					<?php elseif ( $type === 'image' && ! empty( $elem['url'] ) ) : ?>
				...
				<svg viewBox="0 0 100 100" preserveAspectRatio="none" style="position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;overflow:visible;">
					<?php foreach ( $drawings as $stroke ) :
						$pts = $stroke['points'] ?? [];
